...
const productReducer = createSlice({
  ...
  reducers: {
    ...
    productCartAction: (state, action) => {
      const itemCart = state.arrProductCart.find(
...
export const getAllProductAPI = () => {
  return (dispatch) => {
    ...
    const action = productAction(result?.content);
    dispatch(action);
  };
};
...
export const getProductByIdAPI = (id) => {
  return async (dispatch) => {
    const result = productData.content[--id];

    const action = productDetailAction({
      ...result,
      size: result.size.split(","),
      relatedProducts: result.relatedProducts.split(","),
    });
    dispatch(action);
  };
};
...
export const getProductByKeywordAPI = (keyword) => {
  return async (dispatch) => {
    // const result = productData;
    // const action = productAction(result?.content[keyword]);
    const result = await axios({
